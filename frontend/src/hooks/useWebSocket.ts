...
interface WebSocketMessage {
  type: 'monitor_event' | 'task_update' | 'status_update' | 'heartbeat' | 'pong'
  data?: any
}
    ...
    switch (message.type) {
      case 'monitor_event':
        if (message.data) {
          ...
          // Update stats
          setStats({
            totalProductsFound: useStore.getState().stats.totalProductsFound + 1,
            highPriorityFound: message.data.priority === 'high'
