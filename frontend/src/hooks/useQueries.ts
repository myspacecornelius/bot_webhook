...
export function useEngineStatus() {
  return usePolling<EngineStatus>((signal) => api.getStatus(signal), {
    activeInterval: 5000,
...
/**
 * Trending products from intelligence module
 * Polls every 30s when visible, 120s when hidden
 */
export function useTrendingProducts(enabled: boolean = true) {
  return usePolling<{ trending: unknown[] }>(
    (signal) => api.getTrending(signal),
    { activeInterval: 30000, hiddenInterval: 120000 },
    enabled,
  );
}

/**
 * Curated products with profit/priority data
 * Polls every 30s when visible, 120s when hidden
 */
export function useCuratedProducts(enabled: boolean = true) {
  return usePolling<CuratedProductsResponse>(
    (signal) => api.getCuratedProducts(signal),
    { activeInterval: 30000, hiddenInterval: 120000 },
    enabled,
  );
}
...
/**
 * Restock history for calendar views
 * Polls every 60s when visible, 300s when hidden
 */
export function useRestockHistory(hours: number = 24, enabled: boolean = true) {
  return usePolling<{ history: unknown[] }>(
    (signal) => api.getRestockHistory(hours, 50, signal),
    { activeInterval: 60000, hiddenInterval: 300000 },
    enabled,
  );
}
...
export function useAction<T>(action: () => Promise<T>): UseActionResult<T> {
