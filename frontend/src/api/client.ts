...
export const api = {
  ...
  validateLicense: (licenseKey: string) =>
    request<{
      valid: boolean;
      user: { license_key: string; tier: string; expires_at: string };
    }>("/auth/validate", {
      method: "POST",
  ...
  createCheckoutSession: (tier: string, email: string) =>
    request<{ session_id: string; url: string }>("/auth/checkout", {
      method: "POST",
  ...
  startEngine: () => request<{ message: string }>("/start", { method: "POST" }),

  stopEngine: () => request<{ message: string }>("/stop", { method: "POST" }),
  ...
  getMonitorEvents: (limit = 50, signal?: AbortSignal) =>
    request<MonitorEventsResponse>(`/monitors/events?limit=${limit}`, {
      signal,
    }),

  getHighPriorityEvents: (limit = 20, signal?: AbortSignal) =>
    request<MonitorEventsResponse>(
      `/monitors/events/high-priority?limit=${limit}`,
      { signal },
    ),
  ...
  saveMonitorConfig: () =>
    request<{ message: string; count: number }>("/monitors/config/save", {
      method: "POST",
    }),
  ...
  restoreMonitorConfig: () =>
    request<{ message: string; count: number }>("/monitors/config/restore", {
      method: "POST",
    }),
  ...
  getHighPriorityProducts: (signal?: AbortSignal) =>
    request<{ products: CuratedProductsResponse["products"] }>(
      "/products/curated/high-priority",
      { signal },
    ),
  ...
  getRestockHistory: (
    ...
    request<{ history: unknown[] }>(
      `/monitors/restock-history?hours=${hours}&limit=${limit}`,
      { signal },
    ),
  ...
  getRestockPatterns: (days: number = 7, signal?: AbortSignal) =>
    request<{ patterns: unknown[] }>(
      `/monitors/restock-patterns?days=${days}`,
      { signal },
    ),
