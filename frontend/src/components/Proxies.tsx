    ...
    <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-5 hover:border-moss-500/30 transition-all">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-[var(--text)]">{group.name}</h3>
            <p className="text-sm text-[var(--muted)]">{group.proxies.length} proxies</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={onTest}
            className="p-2 rounded-lg bg-[var(--surface2)] text-[var(--muted)] hover:text-[var(--info)] transition-colors"
            title="Test Proxies"
          >
          ...
          <button
            onClick={onDelete}
            className="p-2 rounded-lg bg-[var(--surface2)] text-[var(--muted)] hover:text-red-400 transition-colors"
            title="Delete Group"
          >
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="p-3 rounded-lg bg-[var(--surface2)] text-center">
          <div className="flex items-center justify-center gap-1 mb-1">
            <CheckCircle className="w-4 h-4 text-green-400" />
            <span className="text-lg font-bold text-green-400">{workingCount}</span>
          </div>
          <p className="text-xs text-[var(--muted)]">Working</p>
        </div>
        <div className="p-3 rounded-lg bg-[var(--surface2)] text-center">
          <div className="flex items-center justify-center gap-1 mb-1">
          ...
          <p className="text-xs text-[var(--muted)]">Slow</p>
        </div>
        <div className="p-3 rounded-lg bg-[var(--surface2)] text-center">
          <div className="flex items-center justify-center gap-1 mb-1">
          ...
          <p className="text-xs text-[var(--muted)]">Dead</p>
        </div>
      </div>
        ...
        {group.proxies.slice(0, 5).map((proxy, i) => (
          <div key={i} className="flex items-center justify-between py-1.5 border-b border-[var(--border)] last:border-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-[var(--muted)] font-mono truncate max-w-[200px]">
                {proxy.url.replace(/:[^:]+@/, ':***@')}
              </span>
            </div>
            {proxy.speed && (
              <span className="text-xs text-[var(--muted)]">{proxy.speed}ms</span>
            )}
          </div>
        ))}
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50" onClick={onClose}>
      <div className="w-full max-w-lg bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-6" onClick={e => e.stopPropagation()}>
        <h2 className="text-xl font-bold text-[var(--text)] mb-6">Add Proxy Group</h2>

        <div className="space-y-4">
          <div>
            <label className="block text-sm text-[var(--muted)] mb-2">Group Name</label>
            <input
              type="text"
              value={name}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm text-[var(--muted)]">Proxies (one per line)</label>
              <span className="text-xs text-[var(--muted)]">{proxyCount} proxies</span>
            </div>
            <textarea
              value={proxies}
              onChange={(e) => setProxies(e.target.value)}
              className="w-full h-48 px-4 py-3 bg-[var(--surface2)] border border-[var(--border)] rounded-lg text-[var(--text)] placeholder-gray-500 focus:outline-none focus:border-moss-500 font-mono text-sm resize-none"
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text)] flex items-center gap-3">
            <Globe className="w-7 h-7 text-[var(--info)]" />
            Proxies
          </h1>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            disabled={testing}
            className="flex items-center gap-2 px-4 py-2 bg-[var(--surface2)] text-[var(--muted)] border border-[var(--border)] rounded-lg hover:border-[var(--info)]/30 transition-colors"
          >
