...
class ShopifyStoreMonitor:
    ...
    async def check(self) -> MonitorResult:
        ...
        try:
            ...
            return MonitorResult(
                success=True, products=detected, response_time=time.time() - start_time
            )
        ...
        for product in products_data:
            try:
                ...
                if self.target_sizes:
                    matching_sizes = [
                        s for s in sizes_available if s in self.target_sizes
                    ]
...
class MultiStoreMonitor:
    ...
    def __init__(self, max_concurrent: int = 10):
        ...
        # Callbacks
        self._on_product_found: Optional[
            Callable[[DetectedProduct], Awaitable[None]]
        ] = None
    ...
    def add_store(
        ...
        store = MonitoredStore(name=name, url=url, delay_ms=delay_ms)
    ...
    def set_product_callback(
        self, callback: Callable[[DetectedProduct], Awaitable[None]]
    ):
    ...
    def get_stats(self) -> Dict:
        ...
        for store_id, monitor in self.stores.items():
            stats["stores"][store_id] = {
                ...
                "last_check": monitor.store.last_check.isoformat()
                if monitor.store.last_check
                else None,
            }
...
def create_default_monitor(
    target_sizes: Optional[List[str]] = None, stores: Optional[List[str]] = None
) -> MultiStoreMonitor:
    ...
    DEFAULT_STORES = [
        ...
        # {"name": "Jimmy Jazz", "url": "https://www.jimmyjazz.com", "delay_ms": 3000},  # Acquired by Snipes
