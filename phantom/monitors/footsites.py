...
class FootsiteMonitor:
    ...
    def __init__(
        ...
        if site not in self.SITES:
            raise ValueError(
                f"Unknown site: {site}. Available: {list(self.SITES.keys())}"
            )
    ...
    async def _get_session(self) -> httpx.AsyncClient:
        ...
        if self._session is None or self._session.is_closed:
            # Use 'proxy' (singular) for httpx >= 0.23.0
    ...
    async def search_products(
        self, query: str, page: int = 0, size: int = 60
    ) -> List[Dict]:
        """Search for products"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    "Search failed", status=response.status_code, site=self.site_name
                )
                return []
                ...
                for product in products:
                    ...
                    if parsed and self._is_new_or_restocked(parsed):
                        ...
                        if self.target_sizes:
                            matching = [
                                s
                                for s in parsed.available_sizes
